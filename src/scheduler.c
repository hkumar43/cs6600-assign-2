...
long long int last_accessed_row[MAX_NUM_CHANNELS][MAX_NUM_RANKS][MAX_NUM_BANKS];
...
void init_scheduler_vars()
{
	...
	for(i=0;i< MAX_NUM_CHANNELS; i++){
		...
		counter[i] = (HI_TH + LO_TH)/2;
	        policy[i] = 0 ; // 0 is Open Page and 1 is Closed Page.	
        }
	total_hits = 0 ;
	total_misses = 0;
...
void schedule(int channel)
{
		...
		LL_FOREACH(write_queue_head[channel], wr_ptr)
		{
			if(wr_ptr->command_issuable)
			{
				...
				curr_row = wr_ptr->dram_addr.row;
			       	...
			       	if(last_accessed_row[channel][wr_ptr->dram_addr.rank][wr_ptr->dram_addr.bank] != curr_row){
				        
					hit = 0;
					//Little wastage of space. Can have a pair of values here : Bank and Row
					for(i=0; i<MAX_NUM_BANKS;i++)
                        		{
                            			last_accessed_row[channel][wr_ptr->dram_addr.rank][i] = -1;
                        		}

                        		last_accessed_row[channel][wr_ptr->dram_addr.rank][wr_ptr->dram_addr.bank] = curr_row;
				}
		...
		LL_FOREACH(read_queue_head[channel],rd_ptr)
		{
			...
			{ 
				...
				curr_row = rd_ptr->dram_addr.row;
			       	...
			       	if(last_accessed_row[channel][rd_ptr->dram_addr.rank][rd_ptr->dram_addr.bank] != curr_row){
				        hit = 0;

					//Little wastage of space. Can have a pair of values here : Bank and Row
					for(i=0; i<MAX_NUM_BANKS;i++)
                        		{
                            			last_accessed_row[channel][rd_ptr->dram_addr.rank][i] = -1;
                        		}
                        		last_accessed_row[channel][rd_ptr->dram_addr.rank][rd_ptr->dram_addr.bank] = curr_row;
				}
	...
	if (policy[channel] == 1 && !command_issued_current_cycle[channel]) {
       ...
       if(command_issued_current_cycle[channel]){
		...
		else if(policy[channel] == 1 && hit == 1){
